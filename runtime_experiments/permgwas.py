...
def permgwas(args, number_of_samples=None, number_of_snps=None):
    ...
    start = time.time()
    X, y, K, covs, positions, chrom = prep.load_and_prepare_data(args, number_of_samples, number_of_snps)
    ...
    output, runtime_gwas = gwas.gwas(args, X, y, K, covs)
    ...
    if args.perm is not None:
        ...
        adjusted_p_val, min_p_val, my_seeds, runtime_gwas = gwas.perm_gwas(args, X, y, K, output[:, 1], covs)
